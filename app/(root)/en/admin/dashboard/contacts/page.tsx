import { fetchAllContact } from '@/lib/actions/ContactActions/ContactActions';
import React from 'react'
import Link from 'next/link'
import { ChevronLeft } from 'lucide-react'
import AdminContactFormData from '@/components/ContactForm/AdminContactFormData';
...
async function getData({
    ...
    let res = await fetchAllContact({ pageNo, pageSize });
...
export default async function Page({ searchParams }: { searchParams: { [key: string]: string | undefined } }) {
    ...
    return (
        <div>
            <AdminContactFormData data={data} />
            ...
            <div className="flex w-full justify-between items-center flex-wrap gap-4 px-4 md:px-24 py-4">
                ...
                <div className="flex gap-4 flex-wrap">
                    <Link className='px-4 flex justify-center items-center py-1.5 bg-blue-700 rounded-md text-sm text-white'
                        href={{
                            pathname: '/en/admin/dashboard/contacts',
                            query: {

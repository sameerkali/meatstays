...
const CartPage = () => {
  ...
  return cartItems && cartItems?.length !== 0 ? (
      ...
      <div className="bg-slate-50 w-full lg:flex-1 border border-red-600 rounded-lg p-4">
        ...
        {address && address?.address.length !== 0 && (
          <div className="flex flex-col mt-3 gap-[2px] w-fit p-3 text-[14px] border text-gray-900 border-red-600 rounded-md">
            <p className="text-base underline font-[500] text-red-600">
              Delivery Address
            </p>
            <p>
              Name: <span className="font-[500]">{address?.name}</span>
            </p>
            <p>
              Email: <span className="font-[500]">{address?.email}</span>
            </p>
            <p>
              Mobile: <span className="font-[500]">{address?.mobile}</span>
            </p>
            <p>
              Address: <span className="font-[500]">{address?.address}</span>
            </p>
            <p>
              Landmark: <span className="font-[500]">{address?.landmark}</span>
            </p>
            <p>
              pincode: <span className="font-[500]">{address?.pincode}</span>
            </p>
          </div>
        )}
      </div>
    ...
    <div
      className={`my-3 px-4 md:px-24 flex gap-3 flex-col min-h-[60vh] lg:min-h-[40vh]  justify-center items-center ${poppins.className}`}
    >

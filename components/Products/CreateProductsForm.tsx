...
import { createProduct } from "@/lib/actions/ProductActions/ProductActions";
...
const schema = yup
    .object().shape({
        ...
        images: yup.array().of(
            yup.object().shape({
                name: yup.string().required("Image is required").test(
                    'is-url',
                    'Image must start with "https://"',
                    function (value) {
...
const CreateProductsForm = () => {
    ...
    const processForm: SubmitHandler<FormFields> = async (data) => {
        ...
        let images: String[] | undefined = data?.images?.map((item: any) => item.name);
